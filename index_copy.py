...
for rows in (1, 1):
    out = torch.zeros(rows, 1, 128, dtype=torch.float16, device="spyre")
    src = torch.randn(rows, 1, 128, dtype=torch.float16).to("spyre")
    idx = torch.arange(rows, dtype=torch.int32).to("spyre")
